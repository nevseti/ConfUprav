...
class VFSFile(VFSNode):
    ...
    def __init__(self, name, path, content="", encoding="text"):
        ...
        self.size = len(content) if isinstance(content, str) else len(content)
...
class VirtualFileSystem:
            ...
            if child.tag == 'folder':
                ...
                if encoding == 'base64' and content:
                    try:
                        ...
                        # Пытаемся декодировать как UTF-8, если не выходит - оставляем bytes
                        try:
                            content = decoded.decode('utf-8')
                        except UnicodeDecodeError:
                            content = decoded  # Оставляем как bytes для бинарных файлов
                    except Exception as e:
    ...
    def get_node(self, path):
        """Возвращает VFSNode по указанному пути"""
        if path == "/":
            return self.root
...
class ComLineEm:
    ...
    def _get_display_path(self):
        """Возвращает путь для отображения в приглашении"""
    ...
    def _normalize_path(self, target_path):
        """Нормализует путь (обрабатывает ., .., абсолютные и относительные пути)"""
    ...
    def ls(self, args):
        """Реализация команды ls с поддержкой VFS"""
    ...
    def cd(self, args):
        """Реализация команды cd с поддержкой VFS"""
        if not self.vfs.get_info()['loaded']:
            print("Ошибка: VFS не загружена")
            return False
    ...
    def cal(self, args):
        """Реализация команды cal - вывод календаря"""
        now = datetime.now()
    ...
    def uniq(self, args):
        """Реализация команды uniq - фильтрация повторяющихся строк"""
        if not args:
        ...
        if isinstance(content, bytes):
            print("Ошибка: uniq поддерживает только текстовые файлы")
            return False
    ...
    def uname(self, args):
        """Реализация команды uname - информация о системе"""
        if args:
            print("Команда uname не принимает аргументы (в этой реализации)")
            return True
